...
@Singleton
class UsernameScreen @Inject() constructor(
    ...
    override fun show() {
        ...
        table = Table(assetManager.uiSkin()).apply {
            ...
            // Input field
            add(Label("Please enter your username", assetManager.uiSkin()))
            row()
            val textField = TextField("", assetManager.uiSkin()).apply {
                alignment = 1 // Center text alignment
                maxLength = 12
            }
            add(textField).pad(32F)
            row()

            // Buttons
            val buttonStyle = assetManager.uiSkin().get(TextButton.TextButtonStyle::class.java)
            add(TextButton("Enter", buttonStyle).apply {
                addListener(object : ChangeListener() {
                    override fun changed(event: ChangeEvent?, actor: Actor?) {
                        if (textField.text.isEmpty()) {
                            textField.color = Color(255F, 0F, 0F, 255F)
                            return
                        }

                        gamePref.username = textField.text
